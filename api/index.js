...
async function subscribeToMailchimp(email, entityType, tags) {
...
async function fetchMailchimpMembers(offset = 0, count = 100, entityFilter) {
  ...
  if (entityFilter && entityFilter !== "all") {
    members = members.filter(
      (m) => (m.merge_fields.ENTITY || "").toLowerCase() === entityFilter.toLowerCase()
    );
  }
  ...
  waitlist: router({
    ...
    subscribe: publicProcedure.input(
      z2.object({
        ...
        source: z2.string().optional().default("entity-card")
      })
    ).mutation(async ({ input }) => {
      const tags = [
        "agentcorp-waitlist",
...
Mailchimp Tags: ${tags.join(", ")}`
    ...
    list: adminProcedure.input(
      z2.object({
        ...
        count: z2.number().int().min(1).max(200).default(100),
        entityFilter: z2.string().optional().default("all"),
      ...
      const filtered = input.search ? members.filter(
        (m) => m.email_address.toLowerCase().includes(input.search.toLowerCase()) || (m.merge_fields.ENTITY || "").toLowerCase().includes(input.search.toLowerCase())
      ) : members;
      return {
    ...
    exportCsv: adminProcedure.input(
      z2.object({
        entityFilter: z2.string().optional().default("all")
      })
    ).mutation(async ({ input }) => {
      const { members } = await fetchMailchimpMembers(0, 1e3, input.entityFilter);
      const header = ["Email", "Entity Type", "Tags", "Signed Up At", "Status"];
      const rows = members.map((m) => [
        m.email_address,
        m.merge_fields.ENTITY || "",
        m.tags.map((t2) => t2.name).join("; "),
